                    ...
                    if ip_address:
                        ...
                        if self.ip_attempts[ip_address] >= self.rule['conditions'][0]['count']:
                            ...
                            (timestamp - self.ip_last_attempt_time[ip_address]).seconds <= self.rule['conditions'][0]['timeframe'] :
                                alert=(f"Incident detected: {ip_address} made {self.ip_attempts[ip_address]} failed login attempts within {self.rule['conditions'][0]['timeframe']} seconds in {hostname} for user {user} in {outTimestamp}")
